...
function createBanner( files ) {
	...
	return "/*! <?php echo pkg.title || pkg.name %> - v<?php echo pkg.version %> - " +
		"<?php echo grunt.template.today('isoDate') %>\n" +
		"<?php echo pkg.homepage ? '* ' + pkg.homepage + '\n' : '' %>" +
		"* Includes: " + (files ? fileNames.join(", ") : "<?php echo stripDirectory(grunt.task.current.file.src[1]) %>") + "\n" +
		"* Copyright <?php echo grunt.template.today('yyyy') %> <?php echo pkg.author.name %>;" +
		" Licensed <?php echo _.pluck(pkg.licenses, 'type').join(', ') %> */";
}

grunt.initConfig({
	pkg: "<json:package.json>",
	files: {
		dist: "<?php echo pkg.name %>-<?php echo pkg.version %>",
		cdn: "<?php echo pkg.name %>-<?php echo pkg.version %>-cdn",
		themes: "<?php echo pkg.name %>-themes-<?php echo pkg.version %>"
	},
	meta: {
		banner: createBanner(),
	...
	copy: {
		dist: {
			...
			dest: "dist/<?php echo files.dist %>"
		},
		dist_min: {
			src: "dist/minified/**/*",
			strip: /^dist/,
			dest: "dist/<?php echo files.dist %>/ui"
		},
		dist_css_min: {
		...
		dist_min_images: {
			src: "themes/base/images/*",
			strip: /^themes\/base\//,
			dest: "dist/<?php echo files.dist %>/themes/base/minified"
		},
		cdn: {
			src: [
				"AUTHORS.txt",
		...
		cdn_min: {
			...
			strip: /^dist\/minified/,
			dest: "dist/<?php echo files.cdn %>/ui"
		},
		cdn_min_images: {
			src: "themes/base/images/*",
			strip: /^themes\/base\//,
			dest: "dist/<?php echo files.cdn %>/themes/base/minified"
		},
		cdn_themes: {
			src: "dist/<?php echo files.themes %>/themes/**/*",
			strip: "dist/<?php echo files.themes %>",
			dest: "dist/<?php echo files.cdn %>"
		},
		themes: {
			src: [
			...
			dest: "dist/<?php echo files.themes %>"
		}
	},
	zip: {
		dist: {
			src: "<?php echo files.dist %>",
			dest: "<?php echo files.dist %>.zip"
		},
		cdn: {
			src: "<?php echo files.cdn %>",
			dest: "<?php echo files.cdn %>.zip"
		},
		themes: {
			src: "<?php echo files.themes %>",
			dest: "<?php echo files.themes %>.zip"
		}
	},
	md5: {
		dist: {
			src: "dist/<?php echo files.dist %>",
			dest: "dist/<?php echo files.dist %>/MANIFEST"
		},
		cdn: {
			src: "dist/<?php echo files.cdn %>",
			dest: "dist/<?php echo files.cdn %>/MANIFEST"
		},
		themes: {
			src: "dist/<?php echo files.themes %>",
			dest: "dist/<?php echo files.themes %>/MANIFEST"
		}
	},
	qunit: {
